...
export const cargarTodoEnSede = async (sedeId: string) => {
    ...
    for (const prod of productosParaPrueba) {
        try {
            ...
            const resultado = await processInventoryEntry(prod, sedeId);
            ...
            await updateBranchPrice(resultado.productoId, sedeId, precioAleatorio);
