...
import {
  ...
  AlertTriangle, CheckCircle, ArrowLeft, Bell, Shield, Check, Info,
} from "lucide-react";
import { Page, AuthUser, H9, H7, DEMO_ACCOUNTS } from "../shared";
...
// ─── LoginPage ────────────────────────────────────────────────────────────────
export function LoginPage({ onLogin, onNav }: { onLogin: (u: AuthUser) => void; onNav: (p: Page) => void }) {
  ...
  // ── OTP flow ──
  // phase: null = not started, "email" = verifying email, "phone" = verifying phone
  ...
  const hasContact = regEmail.trim() !== "" || regPhone.trim() !== "";
  const canSubmitReg = acceptTerms && regName.trim() !== "" && hasContact && regPass.length >= 8 && regPass === regConfirmPass;
  ...
  const handleRegisterSubmit = () => {
    ...
    setOtpValue("");
    setOtpError("");
    // Start with email OTP if email provided, else go straight to phone
    if (regEmail.trim() !== "") {
      setOtpPhase("email");
    } else {
      setOtpPhase("phone");
  ...
  const handleOtpVerify = () => {
    ...
    setOtpError("");
    if (otpPhase === "email" && regPhone.trim() !== "") {
      // Email done, now verify phone
      setOtpValue("");
      setOtpPhase("phone");
    } else {
      // All done
  ...
  // Which contact is being verified in the OTP modal
  const otpTarget = otpPhase === "email" ? regEmail : regPhone;
  const otpLabel = otpPhase === "email" ? "correo electrónico" : "número de teléfono";
  const otpIcon = otpPhase === "email" ? <Mail size={26} className="text-[#179150]" /> : <Phone size={26} className="text-[#179150]" />;
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-8 shadow-2xl">
            ...
            <p className="text-sm text-muted-foreground text-center mb-1 leading-relaxed">
              Enviamos un código de 6 dígitos a tu {otpLabel}:
            </p>
            <p className="text-sm font-black text-[#179150] text-center mb-4 truncate">{otpTarget}</p>
            <OtpInput value={otpValue} onChange={setOtpValue} />
            {otpError && (
              <p className="text-red-600 text-xs text-center mb-2 flex items-center justify-center gap-1">
        ...
        {view === "login" && (
          <div className="bg-white rounded-2xl border border-border p-8 shadow-sm">
            ...
            <div className="space-y-4">
              <div>
                <label className={labelCls}>Correo electrónico y/o Número Telefónico</label>
                <div className="relative">
              ...
              {/* Crear cuenta — same style as forgot password */}
            ...
            {regSuccess ? (
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <Mail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
                    <input value={regEmail} onChange={e => setRegEmail(e.target.value)} placeholder="tu@email.com" type="email" className={sharedInput} />
                  </div>
                </div>

                {/* Teléfono */}
                <div>
                  <label className={labelCls}>Número Telefónico</label>
                  <div className="relative">
                    <Phone size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
                    <input value={regPhone} onChange={e => setRegPhone(e.target.value)} placeholder="+58 412-1234567" type="tel" className={sharedInput} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <User size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
                    <input value={regCedula} onChange={e => setRegCedula(e.target.value)} placeholder="V-12345678" className={sharedInput} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className={labelCls}>Confirmar <span className="text-red-500">*</span></label>
                    <div className="relative">
                      <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
                      <input
