...
import { Page, PRODUCTS, CATS, BRAND_SYNONYMS, fmtUSD, effectivePrice } from "../shared";
import { ProductBox } from "./ProductCard";

export function SmartSearch({ searchQuery, setSearchQuery, onNav }: {
        ...
        <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-border rounded-2xl shadow-2xl z-[60] overflow-hidden max-h-[480px] overflow-y-auto">
          {synonymHit && (
            <div className="px-4 py-3 bg-[#e0f5eb] border-b border-[#a7f3d0]">
              ...
              <button onClick={() => doSearch(synonymHit[1][0])} className="flex items-center gap-2 text-sm text-[#006064] hover:text-[#179150] transition-colors">
              ...
              {productMatches.map(p => (
                <button key={p.id} onClick={() => doSearch(p.name)}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-black uppercase truncate" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 900 }}>{p.name}</div>
